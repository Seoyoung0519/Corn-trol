...
Corn-trol은 미디어 소비 문화로 인해 훼손된 현대인의 집중력을 복원하고, 휘발되는 생각들을 AI를 통해 맥락에 맞게 자산화하는 서비스입니다. 
본 백엔드 시스템은 사용자의 음성 기록을 AI 서버(STT)와 연동하여 텍스트로 변환하고, 도메인별(집중, 분석, 연결) 데이터를 안정적으로 처리 및 제공하는 핵심 인프라 역할을 수행합니다.

## 문제 정의

현대인들은 숏폼 등 미디어 과소비로 인해 집중력이 저하되고, 떠오르는 생각들이 쉽게 휘발되는 문제를 겪고 있습니다. 
이를 해결하기 위해 텍스트 입력의 허들을 낮추어 사용자의 음성을 즉각적으로 기록하고, AI를 활용해 파편화된 생각들을 맥락에 맞게 분석하여 개인의 지적 자산으로 연결할 수 있는 시스템이 필요했습니다.

## 주요 기능

- **음성 기록 및 STT 변환:** 클라이언트로부터 전달받은 음성 파일을 AWS S3에 업로드하고, Hugging Face Whisper API를 호출하여 텍스트로 변환합니다. (`WhisperService`, `S3Service`)
- **도메인 기반 데이터 처리:** 집중(Focus), 분석(Analysis), 연결(Connection) 도메인에 따른 비즈니스 로직 및 API를 제공합니다. (`src/main/java/com/corntrol/corntrol/domain/`)
- **보안 및 인증:** JWT 기반의 사용자 인증 및 API 접근 제어를 수행합니다.
- **이메일 알림:** Java Mail Sender를 활용하여 사용자에게 필요한 알림을 발송합니다. (`NotificationController`)
...
- **클라우드 스토리지 생명주기 관리:** 단순히 파일을 업로드하는 것을 넘어, 예외 상황에서의 롤백 로직을 통해 클라우드 리소스 누수를 방지하는 방법을 학습했습니다.
- **CI/CD 파이프라인 최적화:** GitHub Actions와 Docker를 활용해 EC2 환경에 무중단 배포를 구성하며, 환경 변수 주입 및 컨테이너 관리의 중요성을 경험했습니다.
